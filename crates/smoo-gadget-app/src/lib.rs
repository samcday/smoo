...
pub async fn run_from_env() -> Result<()> {
    ...
    let cleaned_args = if auto_pid1 { vec![argv0] } else { raw_args };
    let args = Args::parse_from(cleaned_args.clone());
    let result = run_impl(args, cleaned_args, auto_pid1).await;
...
pub async fn run_with_args(args: Args) -> Result<()> {
    let cleaned_args: Vec<OsString> = std::env::args_os().collect();
    run_impl(args, cleaned_args, false).await
}

async fn run_impl(mut args: Args, cleaned_args: Vec<OsString>, auto_pid1: bool) -> Result<()> {
