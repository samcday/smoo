...
async fn run_dispatcher(
    ...
    loop {
        // Stop/session-change exits intentionally cancel active handlers. The
        // data-plane instance is being torn down, so dropping pump handles
        // closes channels and discards stale wire state.
        ...
        select_biased! {
            ...
            req_opt = next_req => {
                ...
                let handler = build_handler(&handle, &sources, request).await?;
...
async fn build_handler(
    ...
    sources: &Arc<BTreeMap<u32, BlockSourceHandle>>,
    request: Request,
) -> Result<BoxFuture<'static, ()>, HostSessionError> {
    tracing::trace!(
...
async fn handle_write(
    ...
    let block_size = source.block_size();
    // Matches the dispatch-time byte count used to reserve this bulk-IN read.
    let byte_len =
        blocks_to_bytes(request.num_blocks, block_size).map_err(HandlerError::Invalid)?;
