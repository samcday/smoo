...
pub trait BlockSource: Send + Sync {
    ...
    async fn total_blocks(&self) -> io::Result<u64>;

    /// Read one or more blocks starting at `lba` into `buf`.
    async fn read_blocks(&self, lba: u64, buf: &mut [u8]) -> io::Result<usize>;

    /// Write one or more blocks starting at `lba` from `buf`.
    async fn write_blocks(&self, lba: u64, buf: &[u8]) -> io::Result<usize>;
    ...
    async fn discard(&self, _lba: u64, _num_blocks: u32) -> io::Result<()> {
        Ok(())
    }
}
...
impl FileBlockSource {
    ...
    fn ensure_aligned(&self, len: usize) -> io::Result<()> {
...
async fn open_block_file(path: &Path) -> anyhow::Result<OpenedBlockFile> {
