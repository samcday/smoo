...
pub use state_store::{
    ExportFlags, ExportMap, ExportMapRecord, ExportSpec, PersistedExportRecord, StateStore,
};
...
impl SmooGadget {
    ...
    pub async fn write_bulk_buffer(&self, buf: &[u8]) -> Result<()> {
...
impl GadgetDataPlane {
    ...
    pub async fn write_bulk_buffer(&self, buf: &[u8]) -> Result<()> {
        ...
        let mut handle = pool.lock().await.checkout();
        debug_assert!(handle.len() >= len);
        handle.as_mut_slice()[..len].copy_from_slice(buf);
