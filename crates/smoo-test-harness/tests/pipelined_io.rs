...
//!
//!
//! This is a high-volume load test, so it intentionally skips pcap-level
//! request/response balance assertions. The smaller controlled scenarios keep
//! those wire assertions; here fio success, observed wire pipelining when a
//! pcap exists, and clean process/ublk teardown are the gate.
...
async fn pipelined_io() -> Result<()> {
    ...
    if let Some(pcap) = result.pcap_assertions().await? {
        let peak = pcap.peak_inflight();
            ...
            peak >= 2,
            "pipelined_io expected pipelined wire activity (peak_inflight >= 2), got {peak}"
        );
    }
